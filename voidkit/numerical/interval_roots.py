"""Deterministic real interval/root helpers."""
...
def bisect_real(
    ...
    *,
    ...
    """Return midpoint, final left/right bracket, and steps for a sign-changing bracket."""
    if not left < right:
...
def lambertw_real_bracket(y: float, branch: int) -> tuple[float, float]:
    """Return a real branch bracket for ``x*exp(x)=y`` when one exists."""
    ...
    if lower_branch_point < y < 0 and branch == -1:
        ...
        return left, -1.0
    raise ValueError(
        "real Lambert W branch requires branch=0 with y>=-1/e or "
        "branch=-1 with -1/e<y<0"
    )


def x_plus_sin_bracket(y: float) -> tuple[float, float]:
    """A guaranteed bracket for solving ``x + sin(x) = y``."""
    return y - 2.0, y + 2.0
