"""Core GENERIC/metriplectic structure diagnostics.

Scientific arithmetic is extracted from the source helper. Repository artifact routing was intentionally omitted.
`jacobi_residual` is a proxy screen in the source, not a full functional Jacobi-identity evaluator.
"""
...
class EntropyMonitor:
    ...
    def snapshot(self) -> Dict[str, Any]:
        return {'t': self.t[-1] if self.t else 0.0, 'sigma': self.sigma[-1] if self.sigma else 0.0, 'DeltaSigma': self.delta_sigma[-1] if self.delta_sigma else 0.0, 'tol': self.tol, 'n': len(self.t)}
    ...
    def gates(self) -> Dict[str, Any]:
        ...
        return {'sigma_nonneg_ok': sigma_min >= -self.tol, 'DeltaSigma_nonneg_ok': delta_min >= -self.tol, 'sigma_min': sigma_min, 'DeltaSigma_min': delta_min, 'tol': self.tol}

# Naming normalization that makes the source limitation explicit.
jacobi_proxy_residual = jacobi_residual
__all__=["check_antisymmetry","check_symmetry_psd","degeneracy_residuals","bracket_J","jacobi_residual","jacobi_proxy_residual","GenericValidationReport","validate_generic_structure","EntropyMonitor"]
