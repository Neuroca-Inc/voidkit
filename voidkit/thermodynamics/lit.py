"""Linear irreversible thermodynamics primitives extracted from VDM research tooling.
Repository artifact writers are omitted; equations and gate calculations are retained.
"""
...
class BoundaryEntropyFluxMonitor:
    ...
    def update_from_heat_flux(self, q_dot_n: float, T_face: float, area: float, dt: float, sign: float=+1.0) -> Dict[str, float]:
        """
    ...
    def snapshot(self) -> Dict[str, float]:
        return {'t': self.t[-1] if self.t else 0.0, 'Js': self.Js[-1] if self.Js else 0.0, 'n': float(len(self.t)), 'tol': self.tol}
...
__all__=["entropy_production_density","entropy_production_rate","onsager_casimir_residual","curie_mask","apply_curie_zeroing","IsotropicFluidCoeffs","build_L_isotropic_fluid","LITGateReport","gate_report","parity_even","parity_momentum_coupled","BoundaryEntropyFluxMonitor"]
