...
pub fn calculate_habituation_score(recent_count: i32, history_length: i32) -> PyResult<f64> {
    if history_length == 0 {
        return Ok(0.0);
    }
...
pub fn calculate_hsi(firing_rates: Vec<f64>, target_var: f64) -> PyResult<f64> {
...
pub fn calculate_total_reward(
    ...
    let reward =
        w_td * td_error_norm + w_nov * novelty_norm - w_hab * habituation_norm + w_hsi * hsi_norm;
