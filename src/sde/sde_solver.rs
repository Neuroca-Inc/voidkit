...
pub fn sde_solver<'py>(
    ...
    for _step in 0..n_steps {
        ...
        let dw: Vec<f64> = (0..n_dim).map(|_| normal.sample(&mut rng)).collect();
    ...
    let states_2d: Vec<Vec<f64>> = states.chunks(n_dim).map(|chunk| chunk.to_vec()).collect();
...
"#;
            ...
            let (times, states) =
                sde_solver(py, drift, diffusion, initial_py.readonly(), t_span, dt).unwrap();
