...
pub fn numerical_integrate(
    ...
    if !a.is_finite() || !b.is_finite() {
        return Err(PyTypeError::new_err("Lower limit 'a' and upper limit 'b' must be finite numbers"));
    }
    
    if a >= b {
        return Err(PyValueError::new_err("Lower limit 'a' must be less than upper limit 'b'"));
    }
    ...
    F: Fn(f64) -> PyResult<f64>,
{
    ...
    if error < tol {
        ...
        let (left_result, left_error) = adaptive_simpson_recursive(
            f, a, c, fa, fc, fd, s_left, abs_tol / 2.0, rel_tol, depth + 1, max_depth
        )?;
        let (right_result, right_error) = adaptive_simpson_recursive(
            f, c, b, fc, fb, fe, s_right, abs_tol / 2.0, rel_tol, depth + 1, max_depth
        )?;
...
mod tests {
    ...
    fn test_sine_integral() {
        ...
        let (result, error) = adaptive_quadrature(&f, 0.0, std::f64::consts::PI, 1e-8, 1e-10).unwrap();
