...
pub fn numerical_ode_solver(
    ...
    if t0 >= tf {
        return Err(PyValueError::new_err(
            "The 't_span' parameter must have t0 < tf.",
        ));
    }
    ...
    if rtol <= 0.0 || atol <= 0.0 {
        return Err(PyValueError::new_err(
            "Tolerances 'rtol' and 'atol' must be positive.",
        ));
    }
    ...
    F: Fn(f64, &[f64]) -> PyResult<Vec<f64>>,
{
    ...
    while t < tf {
        ...
        let y_low: Vec<f64> = y
            ...
            .zip(&k2)
            .zip(&k3)
            .map(|(((yi, k1i), k2i), k3i)| yi + h / 6.0 * (k1i + 4.0 * k2i + k3i))
            .collect();
...
mod tests {
    ...
    fn test_exponential_decay() {
        ...
        let k = 0.1;
        let ode = |_t: f64, y: &[f64]| -> PyResult<Vec<f64>> { Ok(vec![-k * y[0]]) };
