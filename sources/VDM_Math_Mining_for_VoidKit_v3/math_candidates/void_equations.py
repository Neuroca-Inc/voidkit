...
The runtime stencil is part of the discrete operator on the derived carrier M
from CF00. Changing boundary condition or stencil changes transport and must
therefore be tracked explicitly. The stencil is not primitive ontology: CF00
derives the carrier first, then requires runtime support/locality to be computed
from the law's retarded propagator rather than inserted as an adjacency graph.
"""
...
class LatticeEdge:
    """One undirected carrier-stencil edge with transport metadata."""
...
class LatticeDefinition:
    """Finite carrier discretization plus metadata for transport calibration."""
...
def build_ring_lattice_spec(N: int, k: int = 3) -> LatticeDefinition:
    """k-nearest-neighbor ring control discretization."""
    ...
    metadata = {
        "lattice_kind": "ring",
        "cf_stage": "legacy_control_not_cf00_admissible_default",
        "carrier_status": "finite_runtime_control",
        "support_status": "primitive_adjacency_control",
        "dimensions": [N, 1, 1],
        "boundary": "periodic",
        "stencil": f"ring_k{k}",
        ...
        "edge_count": len(edge_list),
        "runtime_admissibility_note": (
            "Ring adjacency is a legacy/control substrate. CF00 admissibility "
            "requires support to be computed from the law's retarded "
            "propagator rather than treated as primitive ontology."
        ),
    }
...
def build_grid_lattice_spec(
    ...
    metadata = {
        "lattice_kind": "grid",
        "cf_stage": "CF00_runtime_discretization",
        "cf_source": "CF00_Induced_Geometry",
        "carrier_status": "discretization_of_derived_carrier_M",
        "support_status": "carrier_stencil_not_primitive_support_graph",
        "dimensions": [Nx, Ny, Nz],
        "boundary": boundary,
        ...
        "dynamic_bond_geometry_mode": "unit_weight",
        "runtime_admissibility_note": (
            "CF00 permits discretizing carrier variables on derived M, but "
            "locality-bearing support must be measured from the discrete "
            "retarded propagator of the law. This stencil is a numerical "
            "operator support, not primitive physical adjacency."
        ),
    }
    return LatticeDefinition(edge_list, metadata)
