...
The input graph in this runtime is the finite carrier stencil used to
discretize the derived carrier M from CF00. It is not a primitive physical
support graph; CF00 support is supposed to be recovered from the retarded
propagator of the discretized law.

INITIAL STATE — DERIVED, not a design choice:

  φ_i(0) = 0.5    All nodes in superposition (unstable vacuum)
  φ̇_i(0) = 0      Field at rest
  ψ_ij(0) = 0.5   Candidate stencil bonds in superposition
...
class Connectome:
    ...
    def __init__(
        ...
        Args:
            N: number of nodes.
            initial_edges: finite carrier stencil — list of (i, j) pairs.
                Provides the numerical derivative support on the derived
                carrier M.  It is not primitive physical support.  The
                PHYSICAL bond pattern (which edges survive/condense) emerges
                from dynamics and must be tested against CF00 support gates.
        """
        self.N = N
