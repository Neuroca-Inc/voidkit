...
def main() -> int:
    ...
    dg = {"g": BranchGenerator("g", PCPermutation((1, 0)), 1)}
    dc = PresentationMonodromyCertifier(2, FinitePresentation(("g",)), dg)
    cex = dc.double_cover_counterexample()
    check("Double cover same stripped visible", cex is not None and cex.same_visible_projection)
    check("Double cover different full register", cex is not None and not cex.same_full_register_transition)
